...
- **Tick rate is 30Hz**, render rate is 60Hz (rAF). Clients interpolate 50ms behind latest snapshot to hide network jitter (`RENDER_DELAY_MS` in `src/main.ts`). (Was bumped to 60Hz briefly in v0.6.2 but reverted in v0.6.5 after sustained server OOMs.)
